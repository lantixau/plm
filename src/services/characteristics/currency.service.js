...
const create = async (body) => {
  if (await Currency.isNameTaken(body.name))
    throw new ApiError(httpStatus.BAD_REQUEST, singularName + ' name already taken');
  const response = await Currency.create(body);
  return response;
};
...
const query = async (filter, options) => {
  const response = await Currency.paginate(filter, options);
  return response;
};
...
const updateById = async (id, updateBody) => {
  ...
  if (updateBody.name && (await Currency.isNameTaken(updateBody.name, id))) {
