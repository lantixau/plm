...
const query = async (filter, options) => {
  const response = await UOM.paginate(filter, options);
  return response;
};
...
const getById = async (id) => {
  return (await UOM.findOne({_id: id}));
};
...
const updateById = async (id, updateBody) => {
  ...
  if (updateBody.name && (await UOM.isNameTaken(updateBody.name, id))) {
